...
function preload() {
  ...
  if (typeof manifestData === "undefined") {
    console.error(
      "エラー: manifest.js が読み込まれていません。index.htmlの順序を確認してください。"
    );
    return;
  }
...
function setup() {
  ...
  uiManager.setup(uiParent, manifestData, {
    // --- プロジェクト保存・読込 ---
    onSave: (name) => {
      let scene = stateManager.createSceneData(
        canvasW,
        canvasH,
        cameraOffset,
        globalScale,
...
function draw() {
  ...
  // ■■■ 修正箇所：無限背景をやめ、保存範囲のガイドを表示 ■■■
  noFill(); // 中身は塗らない
  stroke("#cccccc"); // 薄いグレーの線
  strokeWeight(20); // 太めの枠線にして見やすく
...
function performUndo() {
  let curr = stateManager.createSceneData(
    canvasW,
    canvasH,
    cameraOffset,
    globalScale,
