...
class UIManager {
  ...
  createProjectPanel(parent) {
    ...
    // 画像書き出しエリア
    let imgRow = createDiv().parent(content).style('margin-bottom','15px').style('border-bottom','1px solid #eee').style('padding-bottom','10px');
    createDiv('Export Image:').parent(imgRow).style('font-size','12px').style('color','#666');
    let exportBox = createDiv().parent(imgRow).style('display','flex').style('gap','5px').style('margin-top','5px');
    ...
    let exportBtn = createButton('📷 Save PNG').parent(exportBox).style('flex-grow','1');

    bgPicker.input(() => { if(this.callbacks.onBgChange) this.callbacks.onBgChange(bgPicker.color()); });
    exportBtn.mousePressed(() => { if(this.callbacks.onSaveImage) this.callbacks.onSaveImage(); });

    // プロジェクト保存エリア
    createDiv('Save Project Data:').parent(content).style('font-size','12px').style('color','#666');
    let row1 = createDiv().parent(content).style('display','flex').style('gap','5px').style('margin-bottom','5px');
    let nameInput = createInput('MyChara').parent(row1).style('width','100px');
    let saveBtn = createButton('Save').parent(row1);
    
    saveBtn.mousePressed(() => { if(this.callbacks.onSave) this.callbacks.onSave(nameInput.value()); });

    let loadList = createSelect().parent(content).style('width','100%');
    loadList.option('Select project to load...');
    let loadBtn = createButton('Load Selected').parent(content).style('width','100%').style('margin-top','5px');
    ...
    this.updateProjectList = (projects) => {
        ...
        loadList.option('Select project to load...');
        projects.forEach(p => { loadList.option(p.name); });
    };
  }
  ...
  createSystemPanel(parent) {
    ...
    let row = createDiv().parent(content).style('display','flex').style('gap','5px');
    let undoBtn = createButton('Undo').parent(row);
    let redoBtn = createButton('Redo').parent(row);
    
    undoBtn.mousePressed(() => { if(this.callbacks.onUndo) this.callbacks.onUndo(); });
    redoBtn.mousePressed(() => { if(this.callbacks.onRedo) this.callbacks.onRedo(); });
  }
  ...
  createEditPanel(parent) {
    ...
    let row = createDiv().parent(content).style('display','flex').style('justify-content','space-between').style('margin-bottom','10px');
    let addBtn = createButton('+ Add Char').parent(row);
    let delBtn = createButton('🗑 Delete').parent(row);

    addBtn.mousePressed(() => { if(this.callbacks.onAddChar) this.callbacks.onAddChar(); });
    delBtn.mousePressed(() => { if(this.callbacks.onRemoveChar) this.callbacks.onRemoveChar(); });

    createDiv('Global Zoom:').parent(content).style('font-size','12px');
    let zoomSlider = createSlider(0.5, 3.0, 1.0, 0.1).parent(content).style('width','100%');
    this.elements.globalScale = zoomSlider;

    zoomSlider.input(() => { if(this.callbacks.onZoomChange) this.callbacks.onZoomChange(zoomSlider.value()); });
    ...
    lockCheck.changed(() => { if(this.callbacks.onLockChange) this.callbacks.onLockChange(lockCheck.checked()); });
  }
  ...
  createLayerPanel(parent) {
    ...
    layers.forEach(cat => {
      ...
      let colorContainer = createDiv().class('color-picker-container').parent(content);
      
      if (colorContainer.elt) colorContainer.elt.style.display = 'none'; 
      else colorContainer.style.display = 'none';

      let colorLabel = createDiv('Color Variants:').class('sub-label').parent(colorContainer);
      let colorGrid = createDiv().class('icon-grid color-grid').parent(colorContainer);
  ...
  updateUIState(activeChar, globalZoom) {
    ...
    if(this.elements.lockCheck) this.elements.lockCheck.checked(activeChar.isLocked);
  ...
  updateIconSelection(gridBox, selectedId) {
      ...
      let targetId = (selectedId === null || selectedId === undefined) ? 'null' : selectedId;
